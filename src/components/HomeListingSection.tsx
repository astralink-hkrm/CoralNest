...
import { SKILL_CATEGORIES, type BrowseCategory } from "../lib/categories";
import {
...
export function HomeListingSection({ initialListing = null }: HomeListingSectionProps = {}) {
  ...
  const selectedCategories = useMemo(
    () =>
      categorySlugs.flatMap((slug) => {
        const category = HOME_SKILL_LISTING_CATEGORIES.find((candidate) => candidate.slug === slug);
  ...
  const activeItems = isSearchMode ? filteredSearchSkills : skills;
  const activeStatus = isSearchMode ? searchStatus : status;
  const isEmpty = activeStatus === "idle" && activeItems.length === 0;
  const showSkillStats = !(tab === "trending" && !isSearchMode);
  ...
  useEffect(() => {
    ...
    const cacheKey = listingCacheKey({ kind: "skills", tab, categorySlugs, fetchLimit });
    ...
    const load = fetchSkillListing(tab, categorySlugs, fetchLimit).then((result) => {
  ...
  }, [categorySlugs, fetchLimit, isSearchMode, tab, listingCache]);
  ...
  }, [categorySlugs, fetchLimit, isSearchMode, tab, trimmedSearch]);

  useEffect(() => {
    if (categorySlugs.length === 0) return;
    const validCategorySlugs = categorySlugs.filter((slug) => isSkillCategorySlug(slug));
  ...
  }, [categorySlugs, isSearchMode, tab, trimmedSearch, view]);
    ...
    >
          ...
          <div className="home-v2-listing-actions">
            <div className="home-v2-listing-actions-rail has-category">
              ...
              <HomeListingCategorySelect
                categories={HOME_SKILL_LISTING_CATEGORIES}
                value={categorySlugs}
        ...
        >
          <form className="home-v2-listing-search-bar" onSubmit={handleListingSearchSubmit}>
            ...
            <input
              ...
              placeholder="Search skills on CoralNest"
      ...
      {activeStatus === "idle" && view === "list" && activeItems.length > 0 ? (
        ...
        >
          ...
          <span className="home-v2-listing-head-label">Skill</span>
          {showSkillStats ? <span className="home-v2-listing-head-stat">Popularity</span> : null}
        </div>
      ) : null}

      {activeStatus === "loading" ? (
        <BrowseResultsSkeleton label="Skill" variant="list" />
      ) : null}
      ...
      {activeStatus === "idle" && visibleSkills.length > 0 ? (
        <HomeListingResults
          view={view}
